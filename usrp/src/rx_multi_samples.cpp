...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    std::string args, sync, subdev, channel_list, file, ref ,pps;
    ...
    if (vm.count("subdev"))
        usrp->set_rx_subdev_spec(subdev); // sets across all mboard
    ...
    outfiles.resize(channel_nums.size());

    for(size_t i = 0; i < channel_nums.size(); ++i) {
        outfiles[i].open(file+boost::lexical_cast<std::string>(i) + ".dat", std::ofstream::binary);
    }
    ...
    while (num_acc_samps < total_num_samps) {
        ...
        for(size_t i = 0; i < channel_nums.size(); ++i) {
            // multiply by 2 in the size to account for 1st 4 bytes real 2nd 4 bytes imaginary
            outfiles[i].write((const char*)buff_ptrs[i], samps_per_buff * 2 * sizeof(float));
        }
    ...
    std::cout << boost::format("num_acc_samps: %u") % num_acc_samps << std::endl;
